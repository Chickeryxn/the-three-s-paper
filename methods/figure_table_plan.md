...
### 图3 `fig03_drying_curve_and_sensitivity.png` — 长时程烘干与敏感性
- **类型**：Type 3｜**目标节**：问题2 / 问题3 结果
- **核心声明**：烘干由**内部扩散**控制，判据在 57.46 h 首次越过；烘干时长对表面传质系数 h_m 不敏感、对扩散系数与环境温湿度敏感。
- **面板**：(a) 全域最大干基含水率随时间的曲线 + 0.15 kg/kg 判据线 + 烘干时刻标注；(b) 中心/表面/中间位置时程；(c) 参数与口径对烘干时长的影响（h_m ±5%、D +5%、T\infty +2 °C、C\infty ±0.005、质量方程替代读法）横向条形图。
- **冻结声明**：`q2_drying_time`/`q3_drying_time`(57.4647 h)、`q2_sens_h_m_plus5pct`、`q2_sens_D_plus5pct`、`q2_sens_T_inf_plus2C`、`q2_sens_mass_equation_form`、`q3_C_center_at_end`、`q3_C_surface_at_end`。
- **来源**：`results/Q3/experiments/round1/result3.xlsx`（每 60 s 完整场；Q2 与本问共享同一模型）、`results/Q2/…/metrics/main.json`、`robustness/Q2|Q3/*.json`。
- **图注（草拟）**：*图3 长时程烘干过程与敏感性。(a) 全域最大干基含水率随时间的演化与 0.15 kg/kg 判据；**在题目给定的 h_m、附录3 经验式与恒温段环境口径下**，判据于 57.4647 h 首次越过。(b) 三个位置的含水率时程。(c) 单项扰动对烘干时长的影响：质量方程改用湿基自洽式 −14.7%、环境温度 +2 °C −6.1%、D +5% −4.2%，而 h_m ±5% 仅 ∓0.5%。*
